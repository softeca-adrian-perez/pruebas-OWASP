<?php
echo $this->Html->script('training_trainer.js?v=' . Configure::read('VERSION_CACHE'), array('block' => 'script'));
...
<div class="cnt-breadcrumb">
    ...
    <div>
        ...
        echo $this->Form->button(
            __t('General.Export_trainers'),
            array(
                'class' => 'gd-export-training-trainers-js aag-button medium',
                'value' => 'submit',
                'escape' => false,
                'name' => 'export',
                'data-url' => Router::url(
                    array(
                        ...
                        'action' => 'training_trainers_excel'
                    )
                )
            )
        );
        if (CakeSession::read('Auth.User.role_id') != ConstantsRoles::SUPER_ADMIN) {
            echo $this->Html->link(
                __t('Training.New_training_trainers'),
                array(
                    'controller' => 'trainings_trainers',
                    'action' => 'add'
                ),
...
<div class="cnt-data">
    <div class="aag-padding">
        <?php echo $this->element('../Elements/Comun/trainings_sub_tab', array('selected' => 'trainers')); ?>
    </div>
    <?php echo $this->element('../TrainingsTrainers/Elements/search_training_trainer'); ?>
    <div class="o-auto">
        <table id="training_table" class="table-tracking">
            <thead>
                <tr>
                    <th><?php echo $this->Paginator->sort('TrainingTrainer.name', __t('Training.Trainer_name')); ?></th>
                    <th><?php echo $this->Paginator->sort('TrainingProvider.name', __t('Training.Training_provider')); ?></th>
                    <th><?php echo $this->Paginator->sort('TrainingTrainer.email', __t('Training.Email')); ?></th>
                    <th><?php echo $this->Paginator->sort('TrainingTrainer.phone', __t('Training.Phone')); ?></th>
            ...
            <tbody>
                <?php foreach ($trainings_trainers as $trainer) { ?>
                    <tr>
                        <td>
                            <b><?php echo h($trainer['TrainingTrainer']['name']); ?></b>
                        </td>
                        <td class="color-blue-text">
                            <?php
                                echo $this->Html->link(
                                    '<span>' . $trainer['TrainingProvider']['name'] . '</span>',
                                    array(
                                        'controller' => 'trainings_providers',
                                        'action' => 'home',
                                        '?' => array(
                                            'name' => $trainer['TrainingProvider']['name']
                                        )
                                    ),
                                    array(
                                        'escape' => false,
                        ...
                        <td class="color-blue-text">
                            <?php echo h($trainer['TrainingTrainer']['email']); ?>
                        </td>
                        <td class="color-blue-text">
                            <?php echo h($trainer['TrainingTrainer']['phone']); ?>
                        </td>
                        <?php if (CakeSession::read('Auth.User.role_id') != ConstantsRoles::SUPER_ADMIN) { ?>
                        <td class="td-icons">
                            <?php echo $this->Html->link(
                                '<span class="cursor-pointer edit-trainer aag-icon-editar"></span>',
                                array(
                                    'controller' => 'trainings_trainers',
                                    'action' => 'edit',
                                    $trainer['TrainingTrainer']['id'],
                                ),
                                array(
                                    'class' => 'flex',
                                    'escape' => false,
                            ...
                            ?>
                            <span class="aag-icon-papelera c-fallo delete-trainer cursor-pointer" data-delete-url="
                                <?php echo Router::url(
                                    array(
                                        'controller' => 'trainings_trainers',
                                        'action' => 'ajax_delete',
