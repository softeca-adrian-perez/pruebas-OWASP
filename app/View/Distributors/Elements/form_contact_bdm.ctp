...
<?php echo $this->element('../Distributors/tabs', array('selected' => 'contact_distributor_bdm',)); ?>
<div class="cnt-data p-top-1 p-bottom-1">
    ...
    <div class="cnt-data-element m-top-1">
        <div class="o-auto p-top-1">
            <table id="contact_bdm_table" class="table-tracking tabla-responsive">
                <thead>
                    <tr>
                                ...
                                if ($this->Acceso->haveTradingGroupPermission(ConstantsPermissionsGrouping::EDIT_DISTRIBUTOR, $distributor['Distributor']['trading_group_id'])) {
                                    if ($distributor_id != null) {
                                        ...
                                        foreach ($distributor_contacts as $distributor_contact) {
                                            if ($distributor_contact['DistributorContactBdm']['contact_id'] == $contact['Contact']['id']) {
                                                $exito = true;
                                            }
                                        }
                                        if ($exito) {
                                ?>
                                            <span
                                                data-url='<?php echo Router::url(
                                                                array(
                                                                    'controller' => 'distributors_contacts_bdm',
                                                                    'action' => 'ajax_remove_distributor_contact_bdm',
                                                                    $distributor_id,
                                                                    $contact['Contact']['id']
                                                                )
                                                            ); ?>'
                                                ...
                                                class='aag-icon-agregar-usuario c-exito status-active-bdm'></span>
                                    ...
                                    <?php echo Router::url(
                                                array(
                                                    'controller' => 'distributors_contacts_bdm',
                                                    'action' => 'ajax_add_distributor_contact_bdm',
                                                    $distributor_id,
                                                    $contact['Contact']['id']
                                                )
                                            ); ?>'
                                                data-distributor-id='<?php echo $distributor_id ?>'
                                                data-contact-id='<?php echo $contact['Contact']['id'] ?>'
                                                class='aag-icon-agregar-usuario c-defecto status-inactive-bdm'>
                                            </span>
