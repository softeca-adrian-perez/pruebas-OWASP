...
var GMaps = (function () {
    ...
    var geoLocate = function ($map, distributors) {
        markers = [];
        distributors.forEach(function (point) {
            if (point.Distributor != undefined) {
                if ((Number(point.Distributor.latitude != 0) && (Number(point.Distributor.longitude) != 0))) {
                    var latLng = new google.maps.LatLng(Number(point.Distributor.latitude), Number(point.Distributor.longitude));
                    var routes_tmp = [];
                    ...
                    var marker = new google.maps.Marker({
                        id: point.Distributor.id,
                        position: latLng,
                        map: $map,
                        title: point.Distributor.business_name,
                        icon: '/img/pins/visits/Pin_' + point.Distributor.last_visit + '.png',
                        visible: false,
                        last_visit: point.Distributor.last_visit,
                        client_type: point.Distributor.client_type,
                        city: point.Distributor.town,
                        routes: routes_tmp,
                        in_route: false,
    ...
    var drawRoute = function () {
        ...
        $('.distributor_added').each(function () {
            ...
            distributors_ids.push($fila.data('id'));
        });
        if(waypts.length > 0){
            ...
            markers.forEach(function (marker) {
                if (isInArray(Number(marker.id), distributors_ids)) {
                    hide_markers_route.push(marker);
                }
            });

            distributors_ids.forEach(function (distributor_id) {
                hide_markers_route.forEach(function(marker){
                    if(marker.id == distributor_id){
    ...
    var cargar_info_window = function(marker, distributor_id) {
        var $info = $('#info');
        var url = '/visits/ajax_get_data_infowindow_distributor';
        var data = {};
        ...
        request.done(function (data) {
            ...
            $('#add_to_route').show();
            $('.distributor_added').each(function(){
                if($(this).data('id') == distributor_id){
            ...
            $('#add_to_route').off('click').on('click',function(){
                var url = $('#ajax_get_data_distributor').data('url');
                var data = {};
                data.distributor_id = marker.id;
                var request = PeticionAjax.post(url,data);
                request.done(function(data){
                    const cleanData = DOMPurify.sanitize(data);
                    $('#table-added-distributors').append(cleanData);
                    ...
                    Visit.loadRemoveDistributor();
                    Visit.setOrder();
                    Visit.setOrderTimeByStart();
                    Visit.setOrderTimeByStartCurrentElement($('#table-added-distributors tr:nth-last-child(2)').find('.input_time').first());
                });
                $('#add_to_route').hide();
                $('#remove_from_route').show();
                /*setTimeout(function(){
                    //$('#table-added-distributors').find('.input_time').first().trigger('blur');
                    $(this).prev().trigger('blur');
                }, 250);*/
            });
            $('#remove_from_route').off('click').on('click',function(){
                var distributor_id = marker.id;
                $('.distributor_added').each(function(){
                    if($(this).data('id') == distributor_id){
                        $(this).find('.remove-distributor').trigger('click');
                        $('#add_to_route').show();
